...
class MelanSiteParser(Parser):
    ...
    def parse_ceremony(self) -> dict:
        ...
        date = remove_spaces(self.received_email['content'].split('Ημερομηνία εκδήλωσης:*:')[1].split('Αριθμός ατόμων: *:')[0])
        people = remove_spaces(self.received_email['content'].split('Αριθμός ατόμων: *:')[1].split('Τηλέφωνο επικοινωνίας: *:')[0])
        ceremony_type = remove_spaces(self.received_email['content'].split('Ενδιαφέρεστε για: *:')[1].split('Ημερομηνία εκδήλωσης:*:')[0]) 
